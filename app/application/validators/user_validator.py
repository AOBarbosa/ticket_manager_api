...
from app.application.utils.validation_utils import is_valid_email, only_digits, is_valid_cpf_digits
...
class UserValidator:
    ...
    def validate_found(self, user: Optional[User], *, not_found_detail: str = "Resource not found") -> None:
        if user is None:
    ...
    def validate_create(self, user: User) -> None:
        ...
        if not user.last_name.strip():
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="last_name is required")
        if not user.cpf.strip():
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="cpf is required")
        if not user.date_of_birth:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="date_of_birth is required")
        ...
        self.__validate_cpf(normalized_cpf)
    ...
    def __validate_cpf(self, cpf_to_validate: str) -> None:
        if is_valid_cpf_digits(cpf_to_validate):
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="CPF is invalid")
